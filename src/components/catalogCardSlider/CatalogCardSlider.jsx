...
export default function CatalogCardSlider(props) {
    const { title, category, type, params } = props;
    const [items, setItems] = useState(null);
    const Navigate = useNavigate();
    const fetchDetails=(cetegory,id,title)=>{
        Navigate(`/${category}/details/${id}/${title}`);
    };
    const navigateCatalog = (category, type,title,params) => {
        Navigate(`/${category}/${type}/${title}`,{state:params});
    }
    useEffect(() => {
        const fetchItems = async () => {
            if (params) {
                const response=await config.discover(category,params);
                if (response) {
        ...
        <>
            <div className="card-slider d-flex flex-column mx-5 mt-lg-5 mt-3">
                <div className="slider-title d-flex justify-content-between mb-3 align-items-center">
                    ...
                    <button className="remove-defaults catalog-btn" onClick={() => { navigateCatalog(category,type,title,params) }}>View More</button>
                </div>
                <div className="card-slides d-flex overflow-x-scroll pt-3">
                    {items &&
                        items.map(item => {

                            return (<div className="slide-item me-3 " key={item.id} onClick={()=>{fetchDetails(category,item.id,item.title?item.title:item.name)}}>
                                <div className="card-poster-wrapper   mb-2">
